...
/** One card style: thumb on top, title, optional text. Pass href to make it a blog link. */
export default function BirdCard({ title, text, color = "lavender", href, date, imageUrl }) {
  const classes = `writer-promise-card writer-promise-card--${color}`;

  const content = (
    <>
      {imageUrl ? (
        <div className="writer-card-thumb">
